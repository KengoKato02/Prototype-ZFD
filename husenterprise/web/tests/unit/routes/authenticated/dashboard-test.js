...
module('Integration | Component | stats-wrapper', function(hooks) {
  ...
  test('it renders', async function(assert) {
    ...
    assert.dom('.text-gray-900').includesText('Remaining Emergency Leave Total: 7 Used: 3 Left: 4');
    assert.dom('.text-gray-900').includesText('Used Holidays You have no previous holidays yet');
    assert.dom('.text-gray-900').includesText('Your Holiday Schedule Start by booking a holiday to see');
  });
});
